"""
Comprehensive backtest for BTC and ETH hourly Kalshi strategies (V2).

Out-of-sample period: 2024-01-01 to 2026-04-15
Training period: 2022-01-01 to 2023-12-31

Parameter grid:
  min_ev:          [0.02, 0.03, 0.04, 0.05]
  vol_gate_thresh: [3.5, 4.5, 5.5]
  stake:           $25 (for scale-up reporting)

Reports best config per asset + annualized projections + quarterly breakdown.

Usage:
    python scripts/backtest_hourly_v2.py
    python scripts/backtest_hourly_v2.py --assets ETH
    python scripts/backtest_hourly_v2.py --quick   # test period 2025-01-01 only
"""
...
def grid_search_asset(
    ...
    """Run grid search over min_ev x vol_gate for an asset. Returns best config + all results."""
    ...
    # Fit calibration once on train data with default params
    if verbose:
        print(f"  Fitting calibration on train data...")
    strat_train = make_hourly_strategy(asset, min_ev=0.03, vol_gate_thresh=4.5, stake=stake)
    ...
    for min_ev in min_ev_vals:
        for vol_gate in vol_gate_vals:
            ...
            # Score: annualized PnL, but penalize low trade count and negative Sharpe
            score = m["annualized_pnl"]
            if m["total_trades"] < 20:
                score = -9999  # too few trades to trust
            if m["sharpe"] < 0:
                score *= 0.5   # discount negative Sharpe
...
def main():
    ...
    MIN_EV_VALS    = [0.02, 0.03, 0.04, 0.05]
    VOL_GATE_VALS  = [3.5, 4.5, 5.5]

    print("=" * 70)
    print("  BTC + ETH HOURLY V2 -- COMPREHENSIVE BACKTEST")
    print(f"  Train: {args.train_start} -> {args.train_end}")
    print(f"  Test:  {args.test_start} -> {args.test_end}")
    print(f"  Stake: ${args.stake:.0f} per trade | Seed: {args.seed}")
    print(f"  Grid:  min_ev={MIN_EV_VALS} x vol_gate={VOL_GATE_VALS}")
    print("=" * 70)
    ...
    for asset in args.assets:
        print(f"\n{'='*70}")
        print(f"  {asset} HOURLY V2 GRID SEARCH")
        print(f"{'='*70}")
        ...
        # Scale to different stake sizes
    ...
    print(f"{'='*70}")
    print("  Edge comes from: (1) vol-regime filtering, (2) BTC lead-lag for ETH,")
    print("  (3) distance normalization. Deterministic orderbook = honest PnL.")
    print("  $100k/yr requires ~$1000+ stake and sustained 55%+ WR.")
    print("  Achievable only if live Kalshi spreads match simulator assumptions.")

    out_path = Path("results/hourly_v2_backtest_report.json")
