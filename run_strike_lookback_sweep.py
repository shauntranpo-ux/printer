"""Sweep strike_lookback_bars across all 15m assets."""
import sys, os
...
LOOKBACK_GRID = [10, 15, 20, 30, 45, 60, 90, 120]
...
def run():
        ...
        for lb in LOOKBACK_GRID:
            try:
                df = run_fifteen_min_backtest(
                    ...
                    strike_lookback_bars=lb,
                )
            except Exception as e:
                print(f"  lookback={lb:3d}  ERROR: {e}"); continue

            if df.empty:
                print(f"  lookback={lb:3d}  no trades")
                results.append({"asset": asset, "lookback": lb,
            ...
            print(f"  lookback={lb:3d}  trades={len(df):>5}  "
    ...
    print(f"\n\n{'='*55}")
    print("SUMMARY — Best strike_lookback per asset (by Sharpe)")
    print(f"{'='*55}")
    ...
    for asset in ASSETS:
        ...
        print(f"  {asset}:  best_lookback={int(best['lookback'])}  "
